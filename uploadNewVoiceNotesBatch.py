...
def find_last_filled_row(sheet, column='A'):
    ...
    if last_non_empty > 0:
        # Assuming data starts from column 'A' and extends to column 'Z' or whichever is the last column you expect data in
        last_row_values = sheet.row_values(last_non_empty)
        print(f"Last filled row ({last_non_empty}): {last_row_values}")
...
def uploadCSVtoGsheet(path_to_credentials, sheet_name, csv_file_path):
    try:
        ...
        for chunk in reader:
            # Replace smart quotes and other problematic characters in string columns
            for column in chunk.select_dtypes(include=['object']).columns:  # Focus on string columns
                chunk[column] = chunk[column].str.replace('“', '"').replace('”', '"')
                chunk[column] = chunk[column].str.replace('‘', "'").replace('’', "'")
            ...
            # Debug prints:
            if first_chunk:
                print(f"First line to be uploaded: {data_to_upload[0]}")
                if len(data_to_upload) > 1:
                    print(f"Second line to be uploaded: {data_to_upload[1]}")
                first_chunk = False
